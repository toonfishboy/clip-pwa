...
const PipeCableCalculator: FC = () => {
    ...
    const isPressureError = pipeValues.pressureLoss !== undefined && pipeValues.netPressure !== undefined && pipeValues.pressureLoss > pipeValues.netPressure && selected !== "pressureLoss";

    const result = useMemo(() => {
        const {volume = 0, length = 0, pressureLoss = 0, netPressure = 0, diameter = 0} = pipeValues;
        if (!hasRequiredValues(pipeValues, selected))
            return;
            ...
            <div className={"flex flex-col m-2 gap-2"}>
                <RadioGroup selected={selected}
                    ...
                    <LabelRadioInput number={getCalcValue("volume")} selected={selected}
                                     radioValue={"volume"} label={"Volumen:"}
                                     onNumberChange={updatePipeValues("volume")}/>
                    <LabelRadioInput number={getCalcValue("length")} selected={selected}
                                     radioValue={"length"} label={"Länge [m]:"}
                                     onNumberChange={updatePipeValues("length")}/>
                    <LabelRadioInput number={getCalcValue("pressureLoss")} selected={selected}
                                     radioValue={"pressureLoss"} label={"Druckverluste [bar]:"}
                                     onNumberChange={updatePipeValues("pressureLoss")}/>
                    <LabelRadioInput number={getCalcValue("netPressure")} selected={selected}
                                     radioValue={"netPressure"} label={"Netzdruck [barÜ]:"}
                    ...
                    <LabelRadioInput number={getCalcValue("diameter")} selected={selected}
                                     radioValue={"diameter"} label={"Rohrinnendurchmesser di[mm]:"}
                                     onNumberChange={updatePipeValues("diameter")}/>
                    {!isNaN(result?.pipeVolume ?? NaN) &&
                        <LabelText>
                            Das Rohr hat ein Volumen von: {result?.pipeVolume} Liter
                        </LabelText>}
                    {!isNaN(result?.airSpeed ?? NaN) &&
                        <LabelText>
                            Die Luft Geschwindigkeit beträgt: {result?.airSpeed} [m/s]
                        </LabelText>}
                </RadioGroup>
                <div className={"flex"}>
