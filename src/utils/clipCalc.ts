...
export function pipeCalculator(volume: number, length: number, pressureLoss: number, netPressure: number, diameter: number, selected: SelectedPipeValue) {
...
export function leakageCalculator(pressureP1: number, pressureP2: number, temperature: number, kappa: number, gas: number, hole: number, leakageCurrent: number, toCalc: SelectedLeakageValue): [number, number] {
    ...
    switch (toCalc) {
        case 'leakageCurrent':
            const vv = Math.PI * Math.pow(hole / 1000, 2) / 4 * psi * Math.pow(2 * pressureP1 * 100000 * density, 0.5) / density;
...
export function containerCalculator(deliveredAmount: number, necessaryAmount: number, engineTolerance: number, offPressure: number, onPressure: number, volume: number, toCalc: SelectedContainerValue) {
    volume = volume / 1000;
    switch (toCalc) {
        case "volume":
...
export function containerLeakageCalculator(volume: number, onPressure: number, offPressure: number, measureTime: number) {
    volume = volume / 1000;
...
export function waterAmountCalculator(airTemp: number, humidity: number, airDelivery: number, pressure: number = 1): number {
